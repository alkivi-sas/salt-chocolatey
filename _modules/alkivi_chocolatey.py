...
__virtualname__ = "alkivi_chocolatey"
...
def remove_source(name):
    """
    Instructs Chocolatey to remove a source.
    ...
    .. code-block:: bash
        ...
        salt '*' chocolatey.remove_source <source name> 
...
def list_sources():
    ...
    pkg_re = re.compile(r"(.*)\|(.*)\|(.*)\|(.*)\|.*\|.*\|.*\|(.*)\|(.*)")
    for line in result["stdout"].split("\n"):
        for name, url, disabled, user, self_service, admin_only in pkg_re.findall(line):
            if name not in ret:
                ret[name] = {"URL: ": url, "Disabled": disabled, "User: ": user, "Self-Service": self_service, "Admin-Only": admin_only}

    return ret


def add_features_to_del(name):
...
def _change_feature_state(name, state):
    """
    ...
    """
    cmd = [_find_chocolatey(), "feature", state, "--name", name]
    result = __salt__["cmd.run_all"](cmd, python_shell=False)
...
def enable_feature(name):
    ...
    """
    return _change_feature_state(name, "enable")
